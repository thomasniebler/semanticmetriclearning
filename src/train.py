...
parser.add_argument('-o', '--outputfile', help="path where the transformed vectors should be saved to")
...
parser.add_argument('--evalstepsdir', help="directory where steps are saved to")
...
model = alg.fit(vectors, relscores, eval_steps=args.evalsteps, learning_rate=args.learningrate,
                batchsize=args.batchsize, max_spark_cores=args.maxsparkcores, output_dir=args.evalstepsdir,
...
# if args.outputfile is not None and args.outputfile[-1] != "/":
#    args.outputfile += "/"
