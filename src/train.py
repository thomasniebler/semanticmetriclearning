...
parser.add_argument('relscores', help="a tab separated csv file with word pairs and numeric relatedness scores")
...
relscores = utils.load_eval_df(args.relscores)

alg = rrl.RRL(verbose=args.verbose)
print(str(datetime.now()) + "\tTraining...")
model = alg.fit(vectors, relscores, eval_steps=args.evalsteps)
